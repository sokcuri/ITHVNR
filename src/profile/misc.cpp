...
std::wstring ParseCode(const HookParam& hp)
{
	...
	if (hp.type & NO_CONTEXT)
		code += L'N';
	if (hp.offset >> 31)
		code += L'-' + ToHexString((unsigned)(-(signed)(hp.offset + 4)));
	else
		code += ToHexString(hp.offset);
	if (hp.type & DATA_INDIRECT)
	{
		if (hp.index >> 31)
			code += L"*-" + ToHexString((unsigned)(-(signed)hp.index));
		else
	...
	{
		if (hp.split >> 31)
			code += L":-" + ToHexString((unsigned)(-(signed)(4 + hp.split)));
		else
	...
	if (hp.type & SPLIT_INDIRECT)
	{
		if (hp.split_index >> 31)
			code += L"*-" + ToHexString((unsigned)(-(signed)hp.split_index));
		else
